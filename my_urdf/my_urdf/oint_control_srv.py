...
class MinimalService(Node):
    ...
    def interpolacja_nieliniowa(self,request):
        ...
        while (i<steps):
            ...
            position[3] = a_r*(i*sample_time)**3 + b_r*(i*sample_time)**2 + c_r*(i*sample_time) + d_r
            position[4] = a_p*(i*sample_time)**3 + b_p*(i*sample_time)**2 + c_p*(i*sample_time) + d_p
            position[5] = a_y*(i*sample_time)**3 + b_y*(i*sample_time)**2 + c_y*(i*sample_time) + d_y
